...
**Status: Phase 1 underway — [live at gergol.github.io/strata](https://gergol.github.io/strata/).** Three browser-verified query layers (soil pH via SoilGrids COG, OSM drinking-water points via Overpass, and an hourly same-origin materialization of the Austrian electricity generation mix from Energy-Charts), plus the first descriptor-driven raster overlay using SoilGrids' official WMS. The MapLibre PWA has a worker-hosted query engine, browser-aware scheduled health checks, BYOK settings, and deterministic UI/offline regression tests. WorldCover's official COG bucket is range-readable but omits browser CORS headers; that analytical multi-COG path remains explicitly blocked rather than being reported as functional.
