...
**Status: Phase 1 underway — [live at gergol.github.io/strata](https://gergol.github.io/strata/).** Thirty-seven browser-verified query layers: OpenStreetMap and Wikidata packs, capabilities-validated WFS, sampled Open-Meteo weather/air quality, SoilGrids pH, Austria's hourly materialized electricity mix, and compact same-origin Ookla broadband and Eurostat census-grid indexes for Vienna. The 38-layer catalogue also includes SoilGrids and ESA WorldCover raster overlays. The MapLibre PWA selects and centers the current location on startup, and has searchable lazy result panels, shared provider-level rate limiting, browser-aware scheduled health, reproducible materialization recipes, BYOK settings, and deterministic UI/offline regression tests. WorldCover remains deliberately visual-only because its official multi-file COG distribution omits browser CORS headers.
