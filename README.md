...
**Status: Phase 1 underway — [live at gergol.github.io/strata](https://gergol.github.io/strata/).** Seventeen browser-verified query layers: a 15-layer OpenStreetMap POI pack via Overpass, soil pH via SoilGrids COG, and an hourly same-origin materialization of Austria's electricity generation mix from Energy-Charts. Two raster overlays render SoilGrids pH and ESA WorldCover 2021. The MapLibre PWA selects and centers the current location on startup (with a reusable location control), and has searchable lazy result panels, shared provider-level rate limiting, browser-aware scheduled health, BYOK settings, and deterministic UI/offline regression tests. WorldCover is deliberately visual-only: its official multi-file COG distribution remains unsuitable for browser analytics because it omits CORS headers.
