...
**Status: Phase 1 underway — [live at gergol.github.io/strata](https://gergol.github.io/strata/).** Thirty-three browser-verified query layers: a 15-layer OpenStreetMap POI pack via Overpass, 10 geospatial Wikidata layers via SPARQL, three capabilities-validated WFS layers, three sampled Open-Meteo weather/air-quality layers, soil pH via SoilGrids COG, and an hourly same-origin materialization of Austria's electricity generation mix from Energy-Charts. The 34-layer catalogue also includes raster overlays for SoilGrids pH and ESA WorldCover 2021. The MapLibre PWA selects and centers the current location on startup, and has searchable lazy result panels, shared provider-level rate limiting, browser-aware scheduled health, BYOK settings, and deterministic UI/offline regression tests. WorldCover remains deliberately visual-only because its official multi-file COG distribution omits browser CORS headers.
