...
**Status: Phase 2 underway — [live at gergol.github.io/strata](https://gergol.github.io/strata/).** Phase 1's forty browser-verified query layers span OpenStreetMap and Wikidata packs, WFS, sampled Open-Meteo weather/air quality, SoilGrids, Austria's electricity mix, compact Ookla/Eurostat indexes, and live GBFS bike-share availability. The 43-layer catalogue also includes WorldCover, date-selectable NASA GIBS true colour, and Vienna's official 1971 aerial imagery. Phase 2 starts with a reproducible PMTiles v3 derivation and publication pipeline: pinned inputs, bit-identical rebuild checks, decoded archive verification, and stamped provenance sidecars for Pages or Release assets. The MapLibre PWA selects and centers the current location on startup, and has searchable lazy result panels, descriptor-styled map results, shared provider-level rate limiting, browser-aware scheduled health, BYOK settings, and deterministic UI/offline regression tests.
