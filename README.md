...
**Status: Phase 2 underway — [live at gergol.github.io/strata](https://gergol.github.io/strata/).** Forty-one browser-verified query layers span OpenStreetMap and Wikidata packs, WFS, sampled Open-Meteo weather/air quality, SoilGrids, Austria's electricity mix, compact Ookla/Eurostat indexes, live GBFS bike-share availability, and a locally computed central-Vienna surface viewshed. The 44-layer catalogue also includes WorldCover, date-selectable NASA GIBS true colour, and Vienna's official 1971 aerial imagery. Phase 2 has reproducible PMTiles and COG derivation/publication paths with pinned inputs, bit-identical rebuild checks, structural validation, and stamped provenance sidecars. The MapLibre PWA selects and centers the current location on startup, and has searchable lazy result panels, descriptor-styled map results, shared provider-level rate limiting, browser-aware scheduled health, BYOK settings, and deterministic UI/offline regression tests.
