...
```
> *Schema note (v0.8):* the implemented descriptor schema v1 (`packages/core`) additionally defines optional `nodata` (sentinel filtered before scaling), `browser_access` (plan §5), `location_precision` (decision D5), `search_beyond_tile` (R4.4), `terrain_analysis` (bounded projected-COG viewshed or shadow models), and `params` (adapter-specific configuration). Time-dependent health canaries pin `health_assertion.at_time`. `value_type` is mandatory: it decides whether R6.3 (numeric) or R4.3 (categorical) binds. A layer declaring `overlay` mode also declares a validated rendering contract independent of its analytical adapter. An overlay-only layer uses `health_assertion.expect_overlay: true`; the canary validates a rendered tile rather than inventing a point value. A YAML file may contain one descriptor or a strict `defaults` + `layers` pack; `rate_limit.group` shares politeness and circuit state across layers using one provider. Every `bbox_vector` layer declares `params.protocol: overpass | sparql | wfs`. Overpass and SPARQL select strict spatial query templates; WFS pins its version, feature type, CRS spelling, and axis order, then validates those values plus a GeoJSON result format through `GetCapabilities` before querying.
### 6.1 Requirements
