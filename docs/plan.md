...
**M1.3 — Wikidata SPARQL breadth (2026-08-16).** The A1 `bbox_vector` capability now routes explicitly by descriptor protocol instead of guessing from an endpoint. Its SPARQL subtype accepts a strictly validated WHERE-body template, injects bounded `wikibase:around` point geography or `wikibase:box` tile geography, normalizes labelled entities into GeoJSON with canonical Wikidata links, caps feature lists visibly, and computes exact distinct tile counts. Provider failures retain rate-limit, timeout, schema, and upstream error semantics through the local engine.

The first pack adds 10 live layers: lighthouses, castles, memorials, museums, universities, stadiums, coal-fired power stations, nuclear power plants, observatories, and international airports. Each has a positive canary; the complete pack passed a sequential live sweep plus production-origin browser CORS checks. Direct instance-of classes and bounded radii are deliberate: an early subclass-traversal university query exceeded the public service's timeout, so the shipped provenance describes the narrower classification instead of overstating completeness. The catalogue now contains 28 layers, 27 of them queryable, while remaining usable through search on desktop and mobile. M1.4 generic WFS/INSPIRE is next.

**M1.4 — Generic WFS/INSPIRE breadth (2026-08-16).** The A1 router now has a WFS 1.1/2.0 subtype that caches and validates `GetCapabilities` before querying. Descriptors pin the feature type, version, request CRS spelling, and axis order; capabilities must advertise the feature type, CRS, `GetFeature`, and a supported GeoJSON output. Bounded point queries return labelled, capped GeoJSON with explicit nearest-search semantics. Tile counts use `resultType=hits` instead of downloading geometry. Native projected coordinates are transformed through the existing pinned CRS registry, proven by Vienna's EPSG:31256 service, and malformed coordinates or count envelopes fail as schema errors.

The first WFS pack adds three official browser-direct layers: Dutch INSPIRE Natura 2000 sites, the Netherlands' national cadastral parcel reference map, and Vienna flood-hazard planning zones. The cadastral layer is hard-gated to z18–z22; all three point canaries, exact live tile counts, and production-origin CORS checks pass. The BEV INSPIRE endpoint was also examined as the Austrian cadastre candidate, but its capabilities document advertises product-download operations rather than `GetFeature` or a feature-type list and omits browser CORS. It is therefore recorded as unsuitable for interactive WFS use, not presented as a broken or materialized query layer. No selected service needed the fallback materializer; GML-only or CORS-blocked services still follow the existing drop → materialize → named-shim ladder.
...
**M1.7 — GBFS shared mobility (2026-08-16).** The A1 protocol router now discovers both language-keyed GBFS v2 and direct-feed v3 datasets, validates station information and status, and joins availability by stable station id. Point queries return distance-sorted, visibly capped stations with vehicles, docks, rent/return state, and provider timestamp; tile mode counts the current station snapshot exactly. Discovery URLs are cached, but live status is deliberately fetched per uncached query. Coverage gates prevent querying a city feed from elsewhere.

The first pack adds WienMobil Rad, City Bike Linz, and Nextbike Bratislava from MobilityData-catalogued CC0 feeds. All three pass live data and production-origin CORS checks. Adding another compliant docked system is now a compact descriptor entry; free-floating-only feeds remain a named extension because they use `vehicle_status`/legacy `free_bike_status` rather than station joins. The catalogue now contains 41 layers, 40 queryable. M1.8 GIBS and historical imagery is next.

**M1.8 — Time-aware GIBS and historical imagery (2026-08-16).** Raster overlay contracts now support a daily UTC `{date}` dimension with a bounded date picker and a descriptor-selected default lag; browser health resolves and probes the identical dated tile. Native `max_zoom` now describes source resolution while MapLibre overzooms instead of making low-resolution global imagery disappear at street zoom. Query-result vectors also gained descriptor-driven circle colour, radius, opacity, and stroke, completing M1.1's vector-style branch; the GBFS pack is the live proof.

NASA GIBS MODIS Terra true-colour imagery is live with a selectable recent acquisition day (two-day default lag). The City of Vienna's official CC-BY 1971 25 cm aerial WMTS is the first historical overlay and a concrete input for Phase 2 compare work. Both pass rendered-tile and production-origin CORS checks. The catalogue closes Phase 1 at 43 layers, 40 queryable.

**Phase 1 exit (2026-08-16).** The ≥30 live-layer bar is exceeded. The catalogue answered three cross-domain acceptance questions in real browser runs: current weather/AQI/pollen plus nearby shared bikes; local census population plus quarterly broadband performance; and recent satellite context plus 1971 Vienna aerial imagery. No specialist app among those sources answers the combined question. Scheduled health remains exhaustive and deploy smoke is provider-scoped; green-majority status and any degraded layer stay visible rather than disappearing.
...
**M2.2a — Local surface viewshed (2026-08-16).** The COG adapter now has a strict projected-terrain analysis contract. A point query range-reads only its declared radius, resamples the window to the declared model grid, and runs deterministic per-cell elevation-angle line-of-sight in the existing worker. The result is returned as a modelled, map-first MultiPolygon; descriptor-driven fill styling renders it below labels, while the panel states eye height, grid resolution, radius, visible share, source limitations, and provenance. Terrain computation is zoom-gated at z13+, coverage is inset by the analysis radius, and nodata never becomes claimed visibility.

The first data proof uses the City of Vienna's official CC-BY 2015 airborne-laser surface model tile 35_4. A pinned source-specific recipe verifies the 54 MB archive, average-resamples the 0.5 m Float64 DOM to a 2 m Float32 ZSTD COG, requires a bit-identical output digest, and validates COG structure. The bounded 7.3 MB artifact is committed and served same-origin by Pages; the independent verification workflow rebuilds it byte-for-byte, and production health requires Pages range support. An immutable Release copy remains only a provenance archive because its final asset host did not emit browser CORS. Coverage is deliberately labelled as one central-Vienna cell rather than extrapolated to Austria.

**M2.2b — Date/time surface shadow (2026-08-17).** A second strict terrain-analysis contract now separates the returned radius from a bounded occluder-search distance. For the first Vienna proof the worker reads the same complete 500 m COG window, returns shadow within 250 m on a declared 10 m grid, and caps each ray toward the sun at 250 m. Solar altitude and north-clockwise azimuth come from the maintained dependency-free SunCalc implementation; Strata owns only the surface ray casting. Below-horizon instants return the entire valid model area as shadow, while nodata-crossing daytime rays remain unclassified.

The panel accepts device-local date and time, converts it to an absolute instant, and recomputes in the worker; map fill, shadow share, sun geometry, grid resolution, ray cap, DSM occluders, vintage, and limitations remain visible. Cache keys include the normalized instant, scheduled health pins a deterministic midsummer canary, and the same-origin COG still passes browser range/CORS checks. M2.2 is complete for the bounded central-Vienna proof; expanding terrain shards is a data-coverage package rather than a prerequisite for M2.3 EGMS.
